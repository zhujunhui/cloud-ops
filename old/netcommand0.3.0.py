...
def dotelnet(host, usr, passwd, ensu, vendor='cisco'):
    ...
    if vendor == 'huawei':
        ...
        command = []
        with open('huawei_command.txt', 'r') as CommandFile:
        ...
        # command in telnet session
        command = []
        with open('cisco_command.txt', 'r') as CommandFile:
            for line in CommandFile:
                command.append(line)
        # execute command
        for execommand in command:
...
def devicelist(devicefilename):
    # Open device list file, get device info.
    with open(devicefilename, 'r') as devicefile:
        for line in devicefile:
            ...
            if passwd != '' or vendor != '':
                ...
                print 'Host in file "devicelist.txt" Missing necessary parameters!!!'


if __name__ == '__main__':
    dfilename = 'test.txt'
    devicelist(dfilename)

def main(dfilename='test.txt'):
    devicelist(dfilename)
